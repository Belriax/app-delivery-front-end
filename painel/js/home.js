...
home.method = {
  ...
  carregarGraficoFormaPagamento: (lista) => {
    ...
    let dinheiro = parseFloat(
      (lista[0].dinheiro * 100) / lista[0].total
    ).toFixed(2);
    let credito = parseFloat((lista[0].credito * 100) / lista[0].total).toFixed(
      2
    );
    ...
    const ctx = document
      .getElementById("graficoFormaPagamento")
      .getContext("2d");
  ...
  carregarGraficoTiposEntrega: (lista) => {
    ...
    let delivery = parseFloat(
      (lista[0].delivery * 100) / lista[0].total
    ).toFixed(2);
