...
config.method = {
  ...
  openTab: (tab) => {
    ...
    switch (tab) {
      case 'delivery-retirada':
        config.method.obterCofigTipoEntrega();
        break;
  ...
  obterCofigTipoEntrega: () => {
      ...
      app.method.get('/entrega/tipo', 
        ...
        // validar as configurações do delivery;
  ...
  salvarOpcaoRetirada: () => {
    ...
    if (isNaN(maximo) || maximo < 0){
      app.method.mensagem('Tempo máximo incorreto.');
      return;
    }
  ...
  openTabTaxa: (tab, pai) => {
    ...
    switch (tab) {
      case 'container-sem-taxa':
        config.method.obterCofigSemTaxa();
        break;
  ...
  // obtém as configurações da taxa única 
  listarTaxaUnica: () => {
    TAXA_UNICA_ID = 0;
  ...
  salvarTaxaUnica: () => {
    ...
    let tempominimo = parseFloat(document.querySelector('#txtTaxaUnicaValorTempoMinimoEntrega').value.trim());
    let tempomaximo = parseFloat(document.querySelector('#txtTaxaUnicaValorTempoMaximoEntrega').value.trim());
    ...
    app.method.post('/taxaentregatipo/taxaunica', JSON.stringify(dados),
      (response) => {
        console.log('Dados entrega', response)
        app.method.loading(false);
    ...
    if(list.length > 0) {
      list.forEach((e, i) => {
        ...
        if(e.tempominimo != null && e.tempominimo != '' && e.tempomaximo != null && e.tempomaximo != ''){
          tempo = `de ${e.tempominimo} ate ${e.tempomaximo} min`
        }
  ...
  adicionarTaxaDistancia: () => {
    ...
    let tempominimo = parseFloat(document.querySelector('#txtTaxaDistanciaTempoMinimo').value.trim());
    let tempomaximo = parseFloat(document.querySelector('#txtTaxaDistanciaTempoMaximo').value.trim());

    if(isNaN(distancia) || distancia <= 0) {
      app.method.mensagem('Informe distância corretamente (somente números).')
      return;
    }
    if(isNaN(valor) || valor <= 0) {
      app.method.mensagem('informe o valor da taxa, por favor.')
    ...
    app.method.post('/taxaentregatipo/taxadistancia', JSON.stringify(dados),
      (response) => {
        ...
        document.querySelector("#txtTaxaDistanciaTempoMinimo").value = '';
        document.querySelector("#txtTaxaDistanciaTempoMaximo").value = '';
